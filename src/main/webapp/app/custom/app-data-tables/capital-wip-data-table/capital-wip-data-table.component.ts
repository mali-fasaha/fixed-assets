...
@Component({
  selector: 'gha-capital-wip-data-table',
  templateUrl: './capital-wip-data-table.component.html',
  styleUrls: ['./capital-wip-data-table.component.scss']
})
export class CapitalWipDataTableComponent implements OnInit, OnDestroy {
  capitalWorkInProgresses: ICapitalWorkInProgress[];
  currentAccount: any;
  ...
  loadAmortizationEntities() {
    this.assetAcquisitionDataTableService.fetchAllAssetAcquisitions().subscribe(res => {
      this.capitalWorkInProgresses = res.body;
      this.dtTrigger.next();
    });
  }
  ...
  protected populateAssetAsquisitions(data: IAssetAcquisition[], headers: HttpHeaders) {
    this.capitalWorkInProgresses = data;
  }
}
