...
/**
 * Performance test for the FixedAssetItem entity.
 */
class FixedAssetItemGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the FixedAssetItem entity")
        ...
        .repeat(2) {
            ...
            .get("/api/fixed-asset-items")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new fixedAssetItem")
            .post("/api/fixed-asset-items")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_fixedAssetItem_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created fixedAssetItem")
                .get("${new_fixedAssetItem_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created fixedAssetItem")
            .delete("${new_fixedAssetItem_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
